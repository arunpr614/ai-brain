# Feature Release A17 Bucket Acceptance Manifest Implementation Plan V2
...
Supersedes: `FEATURE_RELEASE_A17_BUCKET_ACCEPTANCE_MANIFEST_IMPLEMENTATION_PLAN_V1_2026-06-16_20-03-00_IST.md`
Adversarial review: `FEATURE_RELEASE_A17_BUCKET_ACCEPTANCE_MANIFEST_IMPLEMENTATION_PLAN_ADVERSARIAL_REVIEW_2026-06-16_20-04-00_IST.md`
...
- Integrate PM sidecar inconsistencies through dated A17 reconciliation notes, preserving historical context.
...
1. Capture current inventory and index state:
   ...
   - compact untracked count derived from `git status --short`
...
6. Incorporate PM sidecar findings:
   ...
   - Add dated A17 reconciliation note that older A12/A4 runtime-pending rows are historical and superseded by A12/A16 evidence where applicable.
   - Add milestone tracker clarification that older Android runtime-pending rows are historical; authenticated Android routes/native note share advanced in A12, while publication gates remain open.
7. Validate A17 docs and trackers:
   ...
   - Accepted path-list inspection to ensure accepted blocks contain concrete file paths only, not directories or globs.
8. Append root `RUNNING_LOG.md` with an A17 milestone entry and verify the latest heading.
9. Capture final post-A17 index state with `git diff --cached --name-only` and verify it matches the pre-A17 state.
...
## Accepted Path-List Rules

- Accepted path-list blocks must contain concrete file paths only.
- Do not put directory paths, wildcard globs, or recursive patterns in accepted path-list blocks.
- Directory paths and glob-like patterns are allowed only in review-required, deferred, or excluded sections.
- `RUNNING_LOG.md` may appear only with an explicit append-only staging policy, not as a normal whole-file accepted source path.
...
- Unsafe-positive scan over A17 docs for false claims of staging, publication, deployment, or completion.
- Tracker presence scan for `A17|Bucket Acceptance`.
- Running-log latest-heading verification.
- Pre/post index-state comparison after all writes.
...
- If accepted path-list blocks contain directories or wildcard globs, A17 is invalid.
...
- If root `RUNNING_LOG.md` is not appended and verified, A17 is incomplete.
