# Feature Release A32 - Roadmap Status Reconciliation PRD v2

Created: 2026-06-17 01:27:00 IST
Owner: Codex
Status: Approved for implementation planning after adversarial review closure
Supersedes: `FEATURE_RELEASE_A32_ROADMAP_STATUS_RECONCILIATION_PRD_V1_2026-06-17_01-25-00_IST.md`
Adversarial review: `FEATURE_RELEASE_A32_ROADMAP_STATUS_RECONCILIATION_PRD_ADVERSARIAL_REVIEW_2026-06-17_01-26-00_IST.md`
...
| --- | --- |
| Pre-existing dirty roadmap edits were not ownership-classified | Adds dirty-diff classification and requires A32 to mark v0.9.8/v0.9.9 roadmap edits as historical context superseded by v0.9.10. |
| Top changelog alone could leave stale lane-summary guidance | Requires a visible UX v2 active-gate row in the version lane summary. |
| Evidence links were missing | Requires links to A31 packet, A7 release packet, delivery tracker, and milestone tracker. |
| Document version could confuse with product versions | Uses `v0.9.10-roadmap` as a document-only version. |
| Staging risk was underspecified | Adds staging allowlist and forbidden-path scan. |
...
`ROADMAP_TRACKER.md` was already dirty before A32. A32 must treat the v0.9.8/v0.9.9 additions as historical status context, not as current release truth. A32 may commit the file only after adding a superseding v0.9.10 current-state entry and confirming that the staged diff does not include unrelated files.
...
| A32-R2 | Preserve older roadmap history rather than rewriting it as if it never existed. | P0 | Older v0.9.8/v0.9.9 entries remain historical and are superseded by v0.9.10. |
...
| A32-R5 | Link to current evidence. | P1 | Roadmap references A31 packet, A7 release packet, delivery tracker, and milestone tracker. |
| A32-R6 | Keep scope to roadmap/status docs. | P0 | No app source, APK, artifacts, Telegram docs, or root running-log staging. |
| A32-R7 | Create an A32 PM update and milestone tracker row. | P1 | Tracker update and milestone row record reconciliation complete. |
| A32-R8 | Append root running log entry. | P1 | Running log records A32 as append-only and remains unstaged unless explicitly approved. |
