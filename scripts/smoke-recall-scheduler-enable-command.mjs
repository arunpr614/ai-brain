...
assert(
  parsed.commands.schedulerEnablement.includes("upsert BRAIN_RECALL_ALLOW_UNVERIFIED_IMPORT 1"),
  "enable command should set the approved unverified Recall import flag",
);
assert(
  parsed.commands.schedulerEnablement.includes("upsert BRAIN_RECALL_ALLOW_METADATA_ONLY_IMPORT 1"),
  "enable command should set the approved metadata-only Recall import flag",
);
assert(
  parsed.commands.schedulerEnablement.includes("upsert BRAIN_RECALL_WARNING_UI_AVAILABLE 1"),
  "enable command should set the warning UI flag for unverified Recall retrieval",
);
assert(
  parsed.commands.emergencyDisable.includes("upsert BRAIN_RECALL_ALLOW_UNVERIFIED_IMPORT 0"),
  "disable command should clear the unverified Recall import flag",
);
assert(
  parsed.commands.emergencyDisable.includes("upsert BRAIN_RECALL_ALLOW_METADATA_ONLY_IMPORT 0"),
  "disable command should clear the metadata-only Recall import flag",
);
assert(
  parsed.commands.emergencyDisable.includes("upsert BRAIN_RECALL_WARNING_UI_AVAILABLE 0"),
  "disable command should clear the warning UI flag",
);
assert(
