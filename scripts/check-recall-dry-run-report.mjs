...
if (!reportPath) {
  fail([{ rule: "missing_report_path", message: "Provide a dry-run report path." }], warnings, null, args);
}
if (!existsSync(reportPath)) {
  fail(
    [{ rule: "missing_report", message: `Dry-run report does not exist: ${reportPath}` }],
    warnings,
    reportPath,
    args,
  );
    ...
    [
      {
        ...
        message: "Dry-run report mtime is more than 1 minute in the future.",
      },
    ],
    warnings,
    reportPath,
...
if (nowMs - stats.mtimeMs > maxAgeMs) {
  fail(
    [
      {
        rule: "stale_report",
        message: `Dry-run report is older than ${args.maxAgeMinutes} minutes.`,
      },
    ],
    warnings,
    reportPath,
...
if (args.requirePrivatePath && !isUnderPrivateReportRoot(reportPath)) {
  findings.push({
    ...
    message: `Dry-run report must stay under ${PRIVATE_REPORT_ROOT}/ when --require-private-path is used.`,
  });
}
  ...
  JSON.stringify(
    {
      ...
      warnings,
      nextGate:
        "Dry-run report passed machine checks. Human review and explicit apply approval are still required before --apply.",
    },
    null,
    2,
  ),
...
function parseArgs(argv) {
  const parsed = {
    ...
    requirePlannedImports: false,
    requirePrivatePath: false,
    failAllSkipped: false,
    allowPolicyBlocks: false,
    allowChangedRemote: false,
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    } else if (arg === "--require-planned-imports") {
      parsed.requirePlannedImports = true;
    } else if (arg === "--require-private-path") {
      parsed.requirePrivatePath = true;
    } else if (arg === "--fail-all-skipped") {
      parsed.failAllSkipped = true;
    } else if (arg === "--allow-policy-blocks") {
      parsed.allowPolicyBlocks = true;
    } else if (arg === "--allow-changed-remote") {
      parsed.allowChangedRemote = true;
...
function validateShape(report, findings, warnings, args) {
  requireEqual(report, "mode", "dry_run", findings);
  ...
  requireEqual(report, "checkpointAdvanced", false, findings);
  requireEqual(report, "cardsImported", 0, findings);
  requireEqual(report, "cardsUpgraded", 0, findings);
  ...
  if (cardsSeen === 0) {
    const message = "Dry-run saw zero Recall cards. This may be valid, but it does not prove import behavior.";
  ...
  if (cardsAvailable !== cardsSeen) {
    findings.push({
      ...
      message: `cardsAvailable ${cardsAvailable} must match cardsSeen ${cardsSeen} before apply.`,
    });
  }

  if (cardsPlannedForImport === 0) {
    const message = "Dry-run plans zero import writes. This may be valid if all cards already exist.";
    if (args.requirePlannedImports) findings.push({ rule: "no_planned_imports", message });
    else warnings.push({ rule: "no_planned_imports", message });
  }

  if (cardsPlannedForImport > args.maxPlannedImports) {
    findings.push({
      rule: "planned_import_cap_exceeded",
      message: `cardsPlannedForImport ${cardsPlannedForImport} exceeds approved cap ${args.maxPlannedImports}.`,
    });
  }

  if (!args.allowPolicyBlocks && cardsBlocked > 0) {
    findings.push({
      rule: "blocked_cards_present",
      message: `cardsBlocked is ${cardsBlocked}; resolve or explicitly allow policy blocks before apply.`,
    });
  }

  if (!args.allowChangedRemote && cardsChangedRemote > 0) {
    findings.push({
      rule: "changed_remote_cards_present",
      message: `cardsChangedRemote is ${cardsChangedRemote}; changed remote content requires review before apply.`,
    });
  }

  const policyBlockTotal = sumRecord(report.policyBlockCounts);
  if (!args.allowPolicyBlocks && policyBlockTotal > 0) {
    findings.push({
      rule: "policy_blocks_present",
      message: `policyBlockCounts total is ${policyBlockTotal}; apply should not proceed.`,
    });
  }

  const blockedByFidelity = getCount(report.plannedActionCounts, "blocked_by_fidelity_policy");
  if (!args.allowPolicyBlocks && blockedByFidelity > 0) {
    findings.push({
  ...
  if (!args.allowChangedRemote && changedRemote > 0) {
    findings.push({
  ...
  if (args.failAllSkipped && cardsSeen > 0 && cardsSkipped === cardsSeen) {
    findings.push({
      ...
      message: "All seen Recall cards were skipped; confirm this is expected before apply.",
    });
  ...
  for (const [key, entryValue] of Object.entries(value)) {
    if (FORBIDDEN_PAYLOAD_KEYS.has(key.toLowerCase())) {
      findings.push({
        ...
        message: `Dry-run report contains forbidden raw payload field: ${path.concat(key).join(".")}.`,
      });
    }
...
function fail(findings, warnings, reportPath, args) {
  console.error("[check:recall-dry-run-report] failed");
  console.error(
    JSON.stringify(
      {
        ok: false,
...
function printHelp() {
  console.log(`Recall dry-run report apply-readiness check

Usage:
  npm run check:recall-dry-run-report -- data/private/recall-live-spikes/dry-run-report.json
  npm run check:recall-dry-run-report -- --report data/private/recall-live-spikes/dry-run-report.json --max-planned-imports 5
...
Default pass criteria:
  ...
  - cardsImported/cardsUpgraded are 0
  - cardsPlannedForImport is <= --max-planned-imports (default 5)
...
Options:
  --report <path>                         Dry-run report path.
  --max-planned-imports <n>               Approved first-apply write cap. Default 5.
  --max-age-minutes <n>                   Report freshness window. Default 120.
  --require-cards-seen                    Fail if cardsSeen is 0.
  --require-planned-imports               Fail if cardsPlannedForImport is 0.
  --require-private-path                  Require the report under ${PRIVATE_REPORT_ROOT}/.
  --fail-all-skipped                      Fail if all seen cards were skipped.
  --allow-policy-blocks                   Permit cardsBlocked and policy block counts.
  --allow-changed-remote                  Permit changed remote planned actions.
