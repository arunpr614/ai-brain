/**
 * API bearer-token auth + rate limiter — v0.5.0 F-016.
 *
 ...
 *     BRAIN_API_TOKEN is absent or < MIN_TOKEN_LENGTH hex chars. This
 ...
 * Override the limit via BRAIN_API_RATE_LIMIT.
 */
...
/**
 * Load the bearer token from env. Uses BRAIN_API_TOKEN.
 */
 ...
 * If BRAIN_API_TOKEN is already set to a valid value, returns false and does
 * nothing. Otherwise generates a fresh 32-byte hex token, writes (or updates)
 * the `BRAIN_API_TOKEN=...` line in `.env` at the repo root, sets
 * `process.env.BRAIN_API_TOKEN` for the running process, and returns true.
 *
 ...
 * file is created with a single `BRAIN_API_TOKEN=...` line.
 *
 ...
 */
export function ensureApiToken(options?: {
  envPath?: string;
  onGenerate?: (token: string) => void;
}): boolean {
  if (loadApiToken() !== null) return false;
 ...
 * equality timing oracle, even though the single-user threat model makes this
 ...
 * home tool. Bounded size — in practice one key (the single API token)
...
function getConfiguredLimit(): number {
  const raw = process.env.BRAIN_API_RATE_LIMIT;
