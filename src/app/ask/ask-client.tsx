...
const EMPTY_INITIAL_MESSAGES: AskInitialMessage[] = [];
...
export function AskClient(props: AskClientProps = {}) {
  const initialMessages = props.initialMessages ?? EMPTY_INITIAL_MESSAGES;
  const stateKey = buildAskClientStateKey(props.threadId, initialMessages);
  return (
    <AskClientInner
      key={stateKey}
      {...props}
...
function askErrorTitle(code: string): string {
  if (code === "LLM_PROVIDER_OFFLINE") return "AI services unavailable";
  return `Error · ${code}`;
}

function askErrorMessage(code: string, message: string | null | undefined): string {
  if (code === "LLM_PROVIDER_OFFLINE") {
    return "AI services are unavailable. Check AI services in Settings, then try again.";
  }
  return message ?? "Ask could not finish. Try again.";
}
