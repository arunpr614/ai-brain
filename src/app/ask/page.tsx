...
export default async function AskPage({
  ...
  if (!verifySessionCookie(c)) {
    redirect(`/unlock?next=${encodeURIComponent(askNextPath({
      scope,
      ids,
      tag,
      topic: topicSlug,
  ...
  return (
    <div className="mx-auto flex h-[calc(100svh-48px)] max-w-[1040px] flex-col px-5 pb-24 pt-6 md:px-6 md:py-8 md:pb-8">
      <h1 className="mb-5 text-[30px] font-semibold leading-[1.2] tracking-[-0.01em] text-[var(--text-primary)]">
        {title}
      </h1>
      {hasMissingScope ? (
        <div className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-4 text-sm text-[var(--text-secondary)]">
          ...
          <Link
            href="/library"
            className="mt-3 inline-flex h-8 items-center rounded-md bg-[var(--action-primary-bg)] px-3 text-sm font-medium text-[var(--action-primary-fg)] hover:bg-[var(--action-primary-bg-hover)]"
          >
            Back to Library
          </Link>
        </div>
