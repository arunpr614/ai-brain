...
**Verified protected-main baseline:** `167a15d57b8f70574a017ea4cda507870f3600d4` on 2026-07-22
**Latest verified deployed application baseline:** `167a15d57b8f70574a017ea4cda507870f3600d4` on 2026-07-22
**NotebookLM rollout boundary:** the protected-main release is deployed with production flags `1:0:0` (UI on, queue off, provider writes off). Extension `0.7.0` is attested and stable-installed locally but not loaded or paired. No target/source creation, signed-in provider canary, or owner-only real-content enablement is inferred.
...
| AI Memory → NotebookLM one-click export | Experimental | High code and UI-only production evidence; live-provider confidence pending | Protected-main code and migration are deployed; authenticated Brain UI exposes the control while production flags remain `1:0:0`, so the queue is paused and provider writes are off | Extension `0.7.0` is attested and stable-installed but not loaded/paired; no target/source, signed-in synthetic canary, or owner-only real-content enablement; undocumented consumer web behavior; no bulk, scheduled, bidirectional, multi-notebook, update, or delete synchronization | `167a15d57b8f70574a017ea4cda507870f3600d4`, migration `026`, production release evidence, NotebookLM API/UI/DB tests, extension attestation and privacy checks |
...
The protected-main baseline and NotebookLM UI-only rollout were verified on 2026-07-22 at `167a15d57b8f70574a017ea4cda507870f3600d4`. The narrow export remains `Experimental`: only the authenticated Brain UI and production safety controls are live, with queue and provider writes off. Runtime evidence is feature-specific; this deployment does not establish a NotebookLM target/source, signed-in provider canary, or real-content export.
...
| AI Brain → NotebookLM synchronization | Deferred | High | A distinct narrow one-item/one-fixed-private-notebook export is deployed UI-only with queue and provider writes off, alongside Markdown export, Recall patterns, and 46/46 research checks | Broad, daily, batch, bidirectional, multi-notebook, update/delete, aggregate/finality/citation, and supported-API contracts remain absent; the narrow experimental UI-only rollout does not satisfy them | 2026-07-21 broad-sync research package plus 2026-07-22 protected-main UI-only export release |
