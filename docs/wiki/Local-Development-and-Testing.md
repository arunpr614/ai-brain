...
Do not treat a green documentation check, extension build, or mocked provider suite as live-provider evidence. The URL-source release passed its full application and extension suites plus protected Product CI. Separate 2026-07-23 production evidence records a one-attempt public YouTube URL request reaching provider `ready` without copied-text fallback; a paired-profile visual source-icon screenshot was not retained.
