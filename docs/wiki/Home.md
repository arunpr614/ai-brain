...
Several capabilities are deliberately not current product behavior. Official caption recovery and owned-media speech-to-text are Inactive. The full Evidence Scan, Reading Studio, Trust Center, spaced repetition, Obsidian sync, and fully offline mobile library remain Planned or Deferred. The relationship graph/connection map is [Deferred by the 2026-07-13 Graphify opportunity council](Graphify-Opportunity-Decision) and is not active for implementation. See [Ideas and Exploration](Ideas-and-Exploration-Catalog).
