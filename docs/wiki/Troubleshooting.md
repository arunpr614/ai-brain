...
| NotebookLM export is unavailable | Settings/item status, runtime tuple, extension version/reload, and connector heartbeat | master/queue/provider control disabled, deployment ceiling closed, extension stale/offline, or connector no longer paired | Changing deployment controls, rebinding, or creating a source is proposed |
