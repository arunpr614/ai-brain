...
**Original rule (superseded):** Only Lane C bumps version + tags releases while v0.6.0 is in flight. Lane L commits land on `main` without bumping.

**Revised rule (2026-05-12, user-approved amendment):** the release-control concern is real but scoped too broadly. Patch-level bug fixes that ship a distinct on-device artifact deserve a visible version label; only minor/major bumps and actual release tags need to stay coordinated.

| Change type | SemVer form | Who may bump | Constraints |
|---|---|---|---|
| **Patch** | `x.y.Z` → `x.y.Z+1` | **Either lane** | Bumping lane must cite the fix commit(s) in the bump commit message AND call out the bump in the next `RUNNING_LOG.md` entry's cross-lane notes. |
| **Minor** | `x.Y.*` → `x.(Y+1).0` | **Lane C only** until v0.6.0 ships | Lane L's features accumulate on `main` at `x.y.z` without a label; Lane C rolls them into the next minor release. |
| **Major** | `X.*.*` → `(X+1).0.0` | **Coordinated via user** | Both lanes pause, user authorizes, one lane executes. |
| **Git tag** (e.g. `git tag v0.6.0`) | — | **Lane C only** until v0.6.0 ships | Patch bumps by Lane L land without tags; Lane C captures them under the next release tag. |

**Rationale:** the original rule's concern was racing on `git tag` + competing version numbers at the v0.6.0 boundary. That risk is real for minor/major bumps and tags, not for patch bumps. Blocking Lane L from stamping a visible label on a P0 bug-fix APK (as happened on 2026-05-12 with the PIN-unlock loop) forced an under-the-table override; the revised rule makes that pattern legitimate and traceable.

**After v0.6.0 ships:** rule collapses — both lanes bump freely, single-lane tagging resumes at whichever lane ships the next minor release.

### First-run of the revised rule (2026-05-12)

Lane L shipped two patch bumps in a single session under the original rule as explicit user-authorized overrides:

- `0.5.1` → `0.5.2` (commit `3137d55`) — SameSite cookie change (theory-incorrect fix, retained as defensive improvement)
- `0.5.2` → `0.5.3` (commit `b844b0f`) — `CapacitorHttp` disable (actual APK unlock-loop fix)

Under the revised rule these are in-scope patch bumps by Lane L without needing explicit override. The cross-lane notes in the running log flag them for Lane C's next catch-up. No tags were cut; Lane C retains release-tag authority.
