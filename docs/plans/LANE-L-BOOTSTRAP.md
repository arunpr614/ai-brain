...
2. **Version bumps follow the tiered rule** (`DUAL-AGENT-HANDOFF-PLAN.md §2`). You MAY bump the patch field (`x.y.Z` → `x.y.Z+1`) for bug fixes — cite the fix commit(s) in the bump commit + flag it in cross-lane notes. You may NOT bump minor (`x.Y.*`) or major fields, and you may NOT cut `git tag` for a release — Lane C retains that until v0.6.0 ships.
