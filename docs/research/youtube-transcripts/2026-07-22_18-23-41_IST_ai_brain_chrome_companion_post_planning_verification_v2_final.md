# AI Brain Chrome Companion - Post-Planning GitHub Verification V2 Final

**Captured:** 2026-07-22 18:23 IST

**Status:** Final decision addendum after adversarial review; fixture/local implementation input, not production approval
...
**Adversarial review:** [`AI_BRAIN_CHROME_COMPANION_POST_PLANNING_VERIFICATION_ADVERSARIAL_REVIEW_2026-07-22_18-31-04_IST.md`](./AI_BRAIN_CHROME_COMPANION_POST_PLANNING_VERIFICATION_ADVERSARIAL_REVIEW_2026-07-22_18-31-04_IST.md)
...
6. The extension sends structured cues and bounded page metadata to the compile-time exact Brain lab origin in research builds. It sends no YouTube cookies, Google account data, browser storage, player response, signed URLs, or YouTube authorization material.
...
The answers are: no top-ten change that alters the recommendation, strong architectural fit, and material corrections for the side-panel manifest, lab-origin authority, and minimum-Chrome lifecycle contract. A full 50-project source refresh was not repeated; rerun discovery immediately before code procurement or a later implementation cycle.

## Adversarial Review Resolution

| Finding | Resolution in V2 |
|---|---|
| P1: production and lab origins were not separated sharply enough | Research builds now require one compile-time exact lab origin, lab extension identity, lab bearer/configuration, and lab data root. Production browser-transcript routes are absent or reject before reading a request body. |
| P2: Chrome 116 does not provide every side-panel lifecycle event | V2 does not rely on `sidePanel.onClosed`, which is Chrome 142+. Transcript text lives only in the side-panel document; minimum/current Chrome close/remount behavior is a packaged test gate. |
| P2: open PR evidence was mutable | PR #42 and PR #48 are pinned to the reviewed head commits below. Material head drift requires reconciliation before implementation. |
| P2: cross-tab/window isolation was implicit | Two-tab, two-window, pinned-panel, moved-tab, grant-loss, navigation, and remount cases are explicit acceptance gates. |
| P2: top-ten refresh was described too broadly | The conclusion is scoped to the previously ranked top ten; full rediscovery remains a procurement gate. |
| P3: official API alternative lacked direct evidence | The matrix now links the official `captions.list` and `captions.download` authorization requirements. |
...
| [PR #42](https://github.com/arunpr614/ai-brain/pull/42), reviewed head [`c22b5aa80bf7`](https://github.com/arunpr614/ai-brain/commit/c22b5aa80bf77f42b6571423299c874c297d0fc5) | Exact-item, per-tab, side-panel recovery prototype and Product Council decision |
| [PR #48](https://github.com/arunpr614/ai-brain/pull/48), reviewed head [`63effafc2c76`](https://github.com/arunpr614/ai-brain/commit/63effafc2c7601dc5ba52df7f0f96fb5af79ae3f) | Held transcript manual-enrichment PRD, UX specification, and implementation plan |
...
PR #42 and PR #48 are open at this snapshot. Their design inputs are reconciled here, but their files are not assumed to be on `main` until merged. Implementation depends on merge of the reviewed or materially equivalent contracts; a changed head requires another reconciliation.

## Current Repository Snapshot

GitHub metadata for the previously ranked top ten was refreshed from the repository API on 2026-07-22. Stars are discovery signals, not quality or safety evidence. This is not a new source audit of every project in the 50-project inventory.

| Rank | Repository | Current head | License signal | Stars | Last push UTC | Refresh result |
|---:|---|---|---|---:|---|---|
| 1 | [`searchpcc/tube2md`](https://github.com/searchpcc/tube2md) | `716654f362ed` | MIT | 1 | 2026-04-27 | Same tested commit; top-ten order unchanged |
| 2 | [`BitYoungjae/just-copy-subtitles-for-youtube`](https://github.com/BitYoungjae/just-copy-subtitles-for-youtube) | `98adb0046c87` | MIT | 1 | 2026-06-30 | Same tested commit; top-ten order unchanged |
| 3 | [`yniijia/subtidex`](https://github.com/yniijia/subtidex) | `8738fa47fdc7` | MIT | 5 | 2026-06-29 | Same tested commit; top-ten order unchanged |
| 4 | [`ANcpLua/yt-transcript`](https://github.com/ANcpLua/yt-transcript) | `3be44284cc19` | MIT | 0 | 2026-07-22 | Four commits ahead; revalidated below; top-ten order unchanged |
| 5 | [`labib2002/Briefly`](https://github.com/labib2002/Briefly) | `2ddf8eeb57d0` | MIT | 4 | 2026-07-10 | Same tested commit; top-ten order unchanged |
...
| Official YouTube Captions API where the user can edit the video | Platform route, not a GitHub extension | OAuth-scoped official API; no page session scraping | `captions.list` requires authorization; `captions.download` requires permission to edit the video | Preferred for owned/authorized editable videos | **Keep as separate first-party route** |

The official API boundary is documented in YouTube's [`captions.list`](https://developers.google.com/youtube/v3/docs/captions/list) and [`captions.download`](https://developers.google.com/youtube/v3/docs/captions/download) references. It is not a generic way to download captions from arbitrary public videos.
...
- `chrome.storage.session` may hold bounded intent/tab metadata in trusted extension contexts only. Transcript text remains only in the side-panel document and is discarded on close/remount, expiry, navigation, cancel, extension reload, or successful commit. It never enters the service worker, `chrome.storage`, or page DOM.
...
- The service worker constructs the compile-time fixed Brain lab URL in research builds and owns the paired lab bearer request. The page and extracted payload cannot select a destination. Production builds expose no browser-transcript destination.
...
- add `scripting`;
- add `sidePanel`;
- retain `activeTab`, `tabs`, storage, context menus, notifications, and alarms;
- compile the research package for one exact Brain lab host, lab extension ID/version, paired lab bearer configuration, and lab data root; do not reuse production capture authority;
- add exact Brain lab-origin `externally_connectable` configuration for the opaque intent handoff;
- keep the production package on its reviewed ordinary Brain host, with browser-transcript intent/inspect/commit code unavailable and server routes absent or rejecting before request-body processing;
- require Chrome 116+ for programmatic tab-specific `sidePanel.open()`, but do not rely on `sidePanel.onClosed`, which is Chrome 142+;
- add no persistent YouTube host permission and no static YouTube content script.

`externally_connectable` is a manifest trust configuration, not a general page-access permission. It must name the configured Brain lab origin exactly and have negative tests for every other origin. Transcript-memory deletion at the Chrome 116 minimum must follow from side-panel document lifetime and explicit state invalidation; the newer close event may be defense in depth only.
...
3. **Tab-bound extension state:** ordinary popup remains unchanged; only a valid intent tab gets the empty action popup and side-panel path; transcript text exists only in that panel document; SPA navigation, close/remount, expiry, tab move/close, and extension reload restore ordinary behavior.
4. **Local packaged MV3 E2E:** exact Brain fixture origin, YouTube-shaped fixture origin, built extension, action click, `activeTab`, side-panel open, Inspect, review, Add, receipt replay, restart, two tabs, two windows, pinned-panel entry, minimum/current supported Chrome, and zero external network requests.
5. **Disabled lab API:** create/claim/authorize-inspect/commit/status contracts on one exact lab origin; strict origin/version/extension checks; hashed expiring intent; exact-item transaction; idempotent receipts; downstream processing hold; production pre-body denial.
...
- separate lab and production manifest snapshots prove the exact host, `externally_connectable`, extension ID/version, build mode, and route contract; they reject `cookies`, `webRequest`, `debugger`, `tabCapture`, `offscreen`, `nativeMessaging`, `<all_urls>`, YouTube host permissions, and static YouTube content scripts;
...
- packaged tests at Chrome 116 and the current supported Chrome prove close/remount cleanup without depending on `sidePanel.onClosed`; if any required cleanup uses that event, the minimum must be raised to Chrome 142;
- request tab versus ordinary tab, two windows, pinned-panel entry, tab detach/move, active-tab grant loss, same-origin/cross-origin navigation, and return-to-tab all preserve one-intent/one-tab isolation or clear state and deny Inspect/Add;
...
- the reviewed PR #42 and PR #48 heads are merged or replaced only by materially equivalent reviewed contracts;
- repository heads, licenses, and the broader discovery set are refreshed again immediately before any code is copied;
...
| Packaged MV3 against separate local Brain-lab/YouTube-shaped fixtures with zero external requests | Conditional go after exact lab/production manifest and lifecycle test review |
...
- Chrome API availability differs across the supported range; minimum-version behavior must remain in CI.
...
- Top-ten repository heads, licenses, activity, archive state, stars, and forks were refreshed through GitHub's repository API.
- The only post-validation leading-repository delta was compared file by file.
- `ANcpLua/yt-transcript@3be44284cc19` passed clean install, lint, build, and 3/3 manifest tests; its audit still reports four high findings. Its live YouTube browser script was not run.
- The refresh CSV parsed as ten unique ranked rows with thirteen columns.
- All local Markdown references in the research index, V1, adversarial review, and V2 resolved.
- AI Brain `git diff --check`, lint, and typecheck passed.
- The complete AI Brain suite passed: 1,034 tests, 0 failures.
- No live YouTube request, transcript extraction, browser-session access, or production request occurred in this verification.
...
Use the exact-item, per-tab side-panel experience from reviewed PR #42 head `c22b5aa80bf7`; the isolated, bounded DOM and atomic hold/receipt architecture from the merged landscape and DOM-capture plan; and the separate processing consent from reviewed PR #48 head `63effafc2c76`. Treat `scripting` plus `sidePanel`, exact lab-origin external messaging, Chrome-minimum lifecycle tests, packaged MV3 local fixtures, and a downstream-processing hold as required architecture, not optional hardening.
