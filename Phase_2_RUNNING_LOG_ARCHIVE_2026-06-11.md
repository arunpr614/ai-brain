# AI Brain - Running Log (ARCHIVED)

> **Archive notice for AI agents:** Do not append new running-log entries to this file. This file is preserved only for historical context through Entry #63 (`2026-06-11 08:37 IST`).
>
> **Active log for every future update:** `Phase_2_RUNNING_LOG.md` in this same folder.
>
> If a user asks to update the running log, add the new entry to `Phase_2_RUNNING_LOG.md`, not to this archived file.
...
## Entry #58 — 2026-06-09 22:45 IST — v0.7.2 production release + APK verification

### What shipped

- **Released v0.7.2 provider guardrails to production** from clean worktree `/tmp/ai-brain-v0.7.2-release` on branch `codex/v0.7.2-release-verify`.
- **Release commit:** `6a892ea` (`chore(android): enforce versioned apk builds`), based on `codex/v0.7.2-stabilization-clean`.
- **Production restart:** `brain.service` active since `Tue 2026-06-09 22:37:51 IST`.
- **Production URL:** `https://brain.arunp.in`.

### Verification completed

- **Local release gates on clean v0.7.2 worktree:**
  - `npm run check:env` PASS.
  - `npm run typecheck` PASS.
  - `npm run lint` PASS with 3 known warnings:
  ...
  - `npm test` PASS: 393 tests, 393 pass.
  - `npm run build` PASS with known `unpdf` `import.meta` warning.
- **Remote provider check before deploy:** PASS.
  - Claude enrichment: HTTP 200.
  - Claude Ask: HTTP 200.
  - Gemini embeddings: returned 768 dimensions.
- **Deploy script:** PASS.
  - Rebuilt production artifact.
  - Synced standalone app to Hetzner.
  - Repaired native dependencies on server.
  - Restarted `brain.service`.
  - Authenticated health check PASS.
  - Remote AI provider check PASS:
    - `anthropic claude-haiku-4-5-20251001` reachable.
    - `gemini gemini-embedding-001` reachable.
- **Production smoke after deploy:** PASS.
  - `/api/health` returned `200` with `{"ok":true}`.
  - `/api/settings/provider-status` returned `llm.status="ok"` and `embed.status="ok"` with no secret values in the response.
  - `/settings` production HTML contains Claude/Gemini provider UI text.
  - `/items/b68aefd0629dd0911c9f0cb9` production HTML contains `Semantic indexing ready`.

### APK verification

- **APK path:** `data/artifacts/brain-debug-v1.0.1-code2.apk`.
- **SHA-256:** `3aed7a1d11e032f8fd62a28b0dc96ef6b30a389bb644c3d27eacfcaee4a5ce92`.
- **Package:** `com.arunprakash.brain`.
- **Version name:** `1.0.1`.
- **Version code:** `2`.
- **Embedded server URL:** `https://brain.arunp.in`.
- **Result:** APK metadata is correct and points at the newly deployed production app.
- **Not completed from this runtime:** physical phone install/smoke, because `adb` is not available here and no installable Android device interface was exposed to the shell.

### Release notes / caveats

- The earlier Google AI Studio billing issue is **resolved for production credentials**: Gemini embedding probe now succeeds and returns the expected vector dimension.
- The local `.env` only contains `BRAIN_API_TOKEN`; it does not contain Claude/Gemini provider keys. Local provider probe therefore falls back to Ollama and fails unless run in warn-only mode. Production provider verification was done against `/etc/brain/.env` on Hetzner instead.
- Production release intentionally used a clean v0.7.2 worktree to avoid mixing the current v0.7.3 Telegram hardening changes into this deployment.
- Telegram live smoke was skipped by deploy script because `TELEGRAM_RELEASE=1` was not set; this was out of scope for the v0.7.2 provider/APK release.

### Next actions

1. **Manual Android smoke:** install `data/artifacts/brain-debug-v1.0.1-code2.apk` on the phone, open Brain, verify Settings shows Claude/Gemini status and item detail shows semantic indexing state.
2. **If the APK is rebuilt for any reason:** bump `versionName`, bump `versionCode`, and keep the version/code in the APK filename.
3. **Next product lane after Android smoke:** Library Offline Reads from DB. Do not revive the historical WorkManager/offline outbox plan.
...
## 2026-06-10 09:09 IST - Entry #59 - Handover pickup: latest APK verified, phone smoke still pending

**Entry author:** Codex
**Triggered by:** user asked to pick up from `Handover_docs/AI_BRAIN_HANDOVER_2026-06-09_23-11-53_IST.md`.

### Done

- Read the handover and confirmed the active project worktree is on `codex/v0.7.3-telegram-capture-hardening` with the v0.7.3 Telegram hardening work still unstaged, uncommitted, unpushed, and intentionally undeployed.
- Confirmed the latest Android artifact exists at `data/artifacts/brain-debug-v1.0.2-code3.apk`.
- Re-verified latest APK metadata:
  - Package: `com.arunprakash.brain`.
  - Version name: `1.0.2`.
  - Version code: `3`.
...
- Checked production provider status; Claude/Anthropic and Gemini embeddings both report `ok`.
- Found Android tooling at `/opt/homebrew/share/android-commandlinetools/platform-tools/adb`, but no Android phone is currently connected.
- Updated `ROADMAP_TRACKER.md` to v0.9.9-roadmap with the corrected latest APK metadata and the still-pending phone smoke caveat.
- Updated `docs/plans/v0.7.2-production-release-and-apk-verification-implementation-plan-2026-06-09_22-26-23_IST.md` so the actionable APK path/version references point to `brain-debug-v1.0.2-code3.apk`.

### Not completed

- Physical Android phone install/smoke is still pending because `adb devices` reports no connected device.
- Android Settings provider status and Android item-detail semantic indexing state have not yet been verified from the APK.
- No staging, commit, push, or deployment was performed.

### Next actions

1. Connect the Android phone with USB debugging trusted.
2. Install `data/artifacts/brain-debug-v1.0.2-code3.apk`.
3. Open Brain, verify production loads, Settings shows Claude/Gemini status, Library opens, and an item detail page shows semantic indexing state.
4. Only after phone smoke passes, start the next product lane: Library Offline Reads from DB for Android.

---

## 2026-06-10 10:11 IST - Entry #60 - Implemented YouTube anti-bot fallback and deploy data-safety guard

**Entry author:** Codex
**Triggered by:** user said `execute` after reviewing `docs/plans/v0.7.4-youtube-capture-recovery-and-data-safety-implementation-plan-2026-06-10_09-59-17_IST.md`.

### Done

- Implemented YouTube InnerTube anti-bot detection for `LOGIN_REQUIRED` responses whose reason includes the sign-in / not-a-bot challenge.
- Added YouTube oEmbed metadata fallback so public YouTube links can still save when production transcript extraction is blocked.
- Added metadata-only captured content with `extraction_warning = youtube_antibot_metadata_only` and a body that clearly says transcript extraction was blocked.
- Updated Telegram URL capture acknowledgement so metadata-only YouTube saves are reported as saved, not as failed.
- Updated Library list warning label to show metadata-only YouTube items clearly.
- Updated item detail warning text to explain YouTube sign-in challenge / metadata-only status in plain language.
- Added `scripts/smoke-youtube-prod.mjs` and `npm run smoke:youtube:prod` for read-only production-host YouTube extractor smoke.
- Strengthened `scripts/deploy.sh` beyond the prior `/data/` rsync exclude by removing `.next/standalone/data` after build, before sync.
- Updated the v0.7.4 implementation plan status to `implemented locally, not committed or deployed`.

### Verification

- `node --import tsx --test src/lib/capture/youtube.test.ts` passed: 24 tests.
- `node --import tsx --test src/lib/telegram/dispatch.test.ts` passed: 14 tests.
- `node --import tsx --test src/lib/items/status.test.ts` passed: 4 tests.
- `bash -n scripts/deploy.sh` passed.
- `npm run smoke:youtube:prod` passed locally with `jNQXAC9IVRw`, warning `null`.
- `npm run typecheck` passed.
- `npm run test` passed: 412 tests.
- `npm run build` passed with the existing `unpdf` warning.
- `npm run smoke:youtube` passed.
- Local default DB remained empty after focused and full test runs: `{items:0,chunks:0,jobs:0}`.
- Current generated `.next/standalone/data` was removed locally after confirming Next still copies it during build.

### Not completed

- No git staging, commit, push, or production deployment was performed.
- No live Telegram message was sent after implementation because the fix is local only.
- Production-host smoke over SSH was not run against the new code because the new code is not deployed.

### Next actions

1. Review the local diff, especially `src/lib/capture/youtube.ts`, `src/lib/telegram/dispatch.ts`, web warning labels, `scripts/deploy.sh`, and `scripts/smoke-youtube-prod.mjs`.
2. Commit intentionally, separating root-cause safety fixes from the YouTube product fallback if practical.
3. Deploy with the updated deploy script.
4. Run `ssh brain 'cd /opt/brain && npm run smoke:youtube:prod'`.
5. Send one affected YouTube link to Telegram and confirm it appears in the web Library as a metadata-only YouTube item when transcript extraction is blocked.

---

## 2026-06-10 23:22 IST - Entry #61 - v0.8 capture-quality release shipped, production smoke green, Next security patch deployed

**Entry author:** AI agent (Codex) · **Triggered by:** user first asked Codex to update a running log, then changed the target to `phase2/Phase_2_RUNNING_LOG.md`.

### Planned since last entry

Baseline from entry #60: YouTube anti-bot fallback and deploy data-safety guard were implemented locally but not committed, pushed, deployed, or smoke-tested through live Telegram/production. The next actions were to review the diff, commit intentionally, deploy, run production YouTube smoke, and send an affected YouTube link through Telegram.

Since entry #60, the user drove a broader sequence: root-cause reports, capture-quality research, spike design/execution, adversarial review, critical hardening, deployment hygiene, production safety gate, v0.8 safe capture upgrades, code review, production deploy, post-deploy smoke, and a follow-up Next.js security patch before starting new feature work.

### Done

- **Root cause and capture-quality research.**
  - Diagnosed the Telegram/YouTube failure: public YouTube links could hit YouTube anti-bot/login-required behavior and were incorrectly surfaced to the user as private/deleted/unavailable.
  - Confirmed the dummy Library records (`Ready`, `Failed`, `Pending`, `Saved`) were not real desired user captures; production cleanup later verified live DB had no such records.
  - Created `ReviewReport/AI_BRAIN_ROOT_CAUSE_REPORT_2026-06-10_09-46-14_IST.md`.
  - Created `ReviewReport/AI_BRAIN_CAPTURE_QUALITY_RECOMMENDATIONS_2026-06-10_10-44-28_IST.md`.
  - Created `ReviewReport/AI_BRAIN_CAPTURE_QUALITY_SPIKE_REQUIREMENTS_2026-06-10_10-50-50_IST.md`.
  - Created spike docs `docs/plans/spikes/SPIKE-006` through `SPIKE-012` covering capture evaluation, YouTube metadata ladder, Substack extraction, LinkedIn safe capture, artifact storage, provider benchmark, and Substack email ingestion.
  - Added spike scripts under `scripts/spikes/` for YouTube, Substack, LinkedIn, artifact storage, extraction-provider benchmark, and Substack email-ingestion exploration.
  - Created `ReviewReport/AI_BRAIN_CAPTURE_QUALITY_SPIKE_EXECUTION_REPORT_2026-06-10_11-16-14_IST.md`.
  - Created `ReviewReport/AI_BRAIN_CAPTURE_QUALITY_SELF_CRITIQUE_2026-06-10_15-15-31_IST.md`.
  - Created `ReviewReport/AI_BRAIN_CAPTURE_QUALITY_HARDENING_EXECUTION_REPORT_2026-06-10_15-46-10_IST.md`.

- **Implementation planning and adversarial review.**
  - Created `docs/plans/v0.7.5-capture-quality-foundation-implementation-plan-2026-06-10_14-22-26_IST.md`.
  - Created `docs/plans/v0.7.6-capture-quality-critical-hardening-implementation-plan-2026-06-10_15-22-19_IST.md`.
  - Created `docs/plans/v0.7.7-deployment-hygiene-implementation-plan-2026-06-10_17-19-11_IST.md`.
  - Created `docs/plans/v0.8.0-capture-quality-next-feature-plan-2026-06-10_17-39-01_IST.md`.
  - Created adversarial review `ReviewReport/AI_BRAIN_V0_8_0_CAPTURE_QUALITY_PLAN_ADVERSARIAL_REVIEW_2026-06-10_17-50-45_IST.md`.
  - Created revised implementation plan `docs/plans/v0.8.0-safe-capture-upgrades-implementation-plan-2026-06-10_17-55-32_IST.md`.

- **Deployment hygiene.**
  - Implemented deployment hygiene on branch `codex/v0.7.7-deployment-hygiene`.
  - Commit `c33166e` (`fix(deploy): use production token for health check`) makes the deploy script use the server's production token safely for health checks.
  - Confirmed the right production-readiness sequence: code review, production DB backup, production-safe API/Telegram smoke, cleanup verification, then deploy/readiness.

- **v0.8 safe capture upgrades.**
  - Created clean temp worktree `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades`.
  - Implemented v0.8 on branch `codex/v0.8.0-safe-capture-upgrades`.
  - Commit `8d689eb07d14afbf0da4718df15c1c14fb3e416e` (`feat(capture): add safe capture upgrade flow`) pushed to origin.
  - Added safe pasted-text upgrade flow:
    - YouTube/Shorts plus meaningful pasted text can save without transcript extraction.
    - Existing weak YouTube/LinkedIn captures can be upgraded with pasted text.
    - Existing strong captures are protected from overwrite/downgrade.
    - Upgrade resets stale summary/quotes/category/enrichment/search/vector state, removes old chunks/vector rows/jobs, clears auto-tags, saves `pre_upgrade_item_json`, and requeues enrichment.
    - Telegram replies now distinguish metadata-only saves, pasted-text saves, and upgrades.
    - Library filters added: All / Needs upgrade / Full text / Metadata only.
    - Item detail exposes the pasted-text upgrade form only where supported.
  - Main files changed in the v0.8 worktree included `src/lib/capture/upgrade-policy.ts`, `src/lib/capture/user-provided.ts`, `src/lib/capture/youtube-user-text.ts`, `src/db/item-upgrades.ts`, `src/app/items/[id]/upgrade-actions.ts`, `src/app/items/[id]/upgrade-text-form.tsx`, `src/lib/capture/artifacts.ts`, `src/lib/capture/capture-url.ts`, `src/lib/capture/quality.ts`, `src/app/api/capture/url/route.ts`, `src/lib/telegram/dispatch.ts`, `src/app/items/[id]/page.tsx`, and `src/components/library-list.tsx`.
  - Created execution report `ReviewReport/AI_BRAIN_V0_8_0_SAFE_CAPTURE_UPGRADES_EXECUTION_REPORT_2026-06-10_20-24-52_IST.md`.

- **Code review and fix.**
  - Ran `toast-developer:review-code`.
  - Review report: `ReviewReport/review-code/codex-v0.8.0-safe-capture-upgrades/2026_06_10_20_31_54.md`.
  - Medium finding: Substack `paywall_preview` items could appear as needing upgrade and show a pasted-text form, but the server only supported pasted-text upgrades for YouTube/Shorts/LinkedIn.
  - Fixed by adding `canUpgradeWithPastedText()` and using it in item page + server action guard.
  - Kept `isNeedsUpgrade()` broad enough for Substack `paywall_preview`, but changed the Substack hint to explain that full Substack text needs newsletter email-body capture later.
  - Validation after fix: tests, typecheck, lint, build, build-artifact check, and `git diff --check` passed. Lint still shows two old unrelated warnings in `src/lib/client/register-sw.ts` and `src/lib/queue/enrichment-batch-cron.ts`.

- **v0.8 production deploy and smoke.**
  - Took production DB backup before deploy: `/opt/brain/data/backups/pre-v0.8-deploy-2026-06-10_21-31-12_IST.sqlite`, integrity `ok`, SHA-256 `472f1d04437bbbe55396b292d62637f1602ffd8f49e86b0fb17aaa759a1cf02f`.
  - Deployed with `BRAIN_DEPLOY_HEALTH_TOKEN_SOURCE=remote BRAIN_AI_PROVIDER_WARN_ONLY=1 ./scripts/deploy.sh`.
  - Deploy passed local release gates, build, artifact check, rsync, remote native dependency repair, service restart, authenticated health check, remote AI provider check, and Telegram webhook reachability.
  - API smoke created and cleaned four captures:
    - YouTube `jNQXAC9IVRw`: `metadata_only`, warning `youtube_antibot_metadata_only`.
    - YouTube Shorts: `metadata_plus_transcript`.
    - LinkedIn with pasted text: `user_provided_full_text`.
    - Substack article: `full_text`.
  - Telegram-shaped webhook smoke created and cleaned the same four capture types.
  - Final v0.8 production sweep: service active, health OK, providers OK, DB/artifact counts clean.

- **Next.js security patch before new feature work.**
  - Production audit after v0.8 found high Next advisory `GHSA-26hh-7cqf-hhc6` affecting `next@16.2.5`, relevant because the app uses Proxy/Middleware auth.
  - Created branch `codex/v0.8.1-next-security-patch` from the deployed v0.8 worktree.
  - Updated `next` to `16.2.9` and `eslint-config-next` to `16.2.9`.
  - Added narrow `overrides.next.postcss = 8.5.15` because Next 16.2.9 still pinned vulnerable PostCSS internally.
  - Commit `8d7faf669df2545481a80c68784e416169b2f5b7` (`fix(deps): patch next security advisory`) pushed to origin.
  - Local validation passed: `npm ci`, production audit clean, typecheck, lint, full tests `453/453`, build, build-artifact check, `git diff --check`.
  - Took valid production backup: `/opt/brain/data/backups/pre-v0.8.1-next-security-patch-2026-06-10_23-01-31_IST.sqlite`, integrity `ok`, size `3608576`, SHA-256 `0d2b3ca864b6e09238532efb95b24aa85e9235586e385a70903ea5efe7b46ece`.
  - Deployed v0.8.1 security patch successfully. Remote `next` now reports `16.2.9`.
  - Production audit now clean: `0` production vulnerabilities.
  - Re-ran API and Telegram-shaped smoke for YouTube, YouTube Shorts, LinkedIn pasted text, and Substack. All passed and were cleaned.
  - Removed one orphan `chunks_vec` row found in final sweep. Final counts clean: `items=0`, `dummy_title_items=0`, `artifacts=0`, `enrichment_jobs=0`, `embedding_jobs=0`, `chunks=0`, `chunks_rowid=0`, `chunks_vec=0`, `telegram_updates=0`, `metadata_cache=0`, `artifact_dirs=0`.

- **Adversarial-review skill.**
  - Created `docs/plans/adversarial-review-skill-implementation-plan-2026-06-10_18-25-40_IST.md`.
  - Created adversarial review `docs/plans/ADVERSARIAL_REVIEW_SKILL_IMPLEMENTATION_PLAN_ADVERSARIAL_REVIEW_2026-06-10_18-31-09_IST.md`.
  - Created revised plan `docs/plans/adversarial-review-skill-revised-implementation-plan-2026-06-10_19-12-41_IST.md`.
  - Installed reusable skill at `/Users/arun.prakash/.codex/skills/adversarial-review/` with `SKILL.md`, `agents/openai.yaml`, `references/report-template.md`, and `scripts/report_path.py`.
  - Created `docs/plans/adversarial-review-skill-install-manifest-2026-06-10_19-58-58_IST.md`.
  - Created second review report `docs/plans/ADVERSARIAL_REVIEW_SKILL_IMPLEMENTATION_PLAN_ADVERSARIAL_REVIEW_2026-06-10_19-52-38_IST.md`.

### Cross-lane notes

- The deployed v0.8/v0.8.1 source of truth is the clean temp worktree `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades`, not the dirty `phase2` worktree.
- `phase2` remains on `codex/v0.7.7-deployment-hygiene` with modified/untracked docs and artifacts. Do not reset or clean it casually.
- Production server is `brain`; app URL is `https://brain.arunp.in`; DB path `/opt/brain/data/brain.sqlite`; capture artifact root `/opt/brain/data/artifacts/captures`.
- Secrets were kept server-side and not printed/persisted.

### Learned

- Public YouTube videos can fail transcript extraction because of YouTube anti-bot/login-required responses. That is not the same as private/deleted/unavailable and should save metadata-only with a clear warning.
- Pasted text is the safest high-quality fallback for YouTube and LinkedIn because it avoids scraping/auth ambiguity and lets the user decide what should be remembered.
- Substack needs a different upgrade path. `paywall_preview` belongs in "Needs upgrade", but showing a pasted-text upgrade form is misleading until email-body capture or another Substack-specific full-text path exists.
- `npm audit --omit=dev` is the production gate that matters for deployed risk. Full local audit still has dev-only moderate advisories, but production install/audit is clean after the Next patch.
- Cleanup scripts that touch `chunks_vec` must load `sqlite-vec` through Node; plain `sqlite3` can fail with `no such module: vec0`.
- Release work in a clean temp worktree prevented accidental mixing with the dirty `phase2` branch.

### Deployed / Released

- **v0.8.0 capture-quality release:** branch `codex/v0.8.0-safe-capture-upgrades`, commit `8d689eb07d14afbf0da4718df15c1c14fb3e416e`, pushed and deployed.
- **v0.8.1 Next security patch:** branch `codex/v0.8.1-next-security-patch`, commit `8d7faf669df2545481a80c68784e416169b2f5b7`, pushed and deployed.
- **Production runtime:** `brain.service` active; authenticated health OK; Anthropic Claude Haiku 4.5 and Gemini embeddings OK; Next `16.2.9`; production audit `0`.
- **Production smoke:** API and Telegram-shaped capture smoke green for YouTube, YouTube Shorts, LinkedIn pasted text, and Substack.

### Documents created or updated this period

**Created:**
- `ReviewReport/AI_BRAIN_ROOT_CAUSE_REPORT_2026-06-10_09-46-14_IST.md` - Telegram/YouTube capture and dummy Library root-cause report.
- `ReviewReport/AI_BRAIN_CAPTURE_QUALITY_RECOMMENDATIONS_2026-06-10_10-44-28_IST.md` - capture-quality recommendations.
- `ReviewReport/AI_BRAIN_CAPTURE_QUALITY_SPIKE_REQUIREMENTS_2026-06-10_10-50-50_IST.md` - spike requirements.
- `ReviewReport/AI_BRAIN_CAPTURE_QUALITY_SPIKE_EXECUTION_REPORT_2026-06-10_11-16-14_IST.md` - spike execution report.
- `ReviewReport/AI_BRAIN_CAPTURE_QUALITY_SELF_CRITIQUE_2026-06-10_15-15-31_IST.md` - adversarial self-critique.
- `ReviewReport/AI_BRAIN_CAPTURE_QUALITY_HARDENING_EXECUTION_REPORT_2026-06-10_15-46-10_IST.md` - hardening execution report.
- `ReviewReport/AI_BRAIN_V0_8_0_CAPTURE_QUALITY_PLAN_ADVERSARIAL_REVIEW_2026-06-10_17-50-45_IST.md` - v0.8 plan adversarial review.
- `docs/plans/v0.7.5-capture-quality-foundation-implementation-plan-2026-06-10_14-22-26_IST.md` - capture-quality foundation plan.
- `docs/plans/v0.7.6-capture-quality-critical-hardening-implementation-plan-2026-06-10_15-22-19_IST.md` - critical hardening plan.
- `docs/plans/v0.7.7-deployment-hygiene-implementation-plan-2026-06-10_17-19-11_IST.md` - deployment hygiene plan.
- `docs/plans/v0.8.0-capture-quality-next-feature-plan-2026-06-10_17-39-01_IST.md` - v0.8 next-feature plan.
- `docs/plans/v0.8.0-safe-capture-upgrades-implementation-plan-2026-06-10_17-55-32_IST.md` - v0.8 safe capture upgrade plan.
- `docs/plans/spikes/SPIKE-006..SPIKE-012-*` - seven capture-quality spike design docs.
- `docs/plans/adversarial-review-skill-implementation-plan-2026-06-10_18-25-40_IST.md` - adversarial-review skill plan.
- `docs/plans/adversarial-review-skill-revised-implementation-plan-2026-06-10_19-12-41_IST.md` - revised adversarial-review skill plan.
- `docs/plans/adversarial-review-skill-install-manifest-2026-06-10_19-58-58_IST.md` - installed-skill manifest.
- `/Users/arun.prakash/.codex/skills/adversarial-review/` - reusable Codex skill.

**Updated:**
- v0.8 release worktree source files under `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/src/` - safe capture upgrade flow, Library filters, item detail upgrade form, Telegram wording, artifact/upgrade handling.
- `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/package.json` and `package-lock.json` - Next/security patch to `16.2.9`, matching `eslint-config-next`, and PostCSS override.
- `Phase_2_RUNNING_LOG.md` - this entry.

### Current remaining to-do

1. **Canonical branch integration:** decide whether to merge/pull `codex/v0.8.1-next-security-patch` into the canonical development branch/main before starting the next feature.
2. **Clean new-feature base:** begin new work from deployed v0.8.1 state, ideally in a fresh branch/worktree, not from the dirty `phase2` branch.
3. **Substack upgrade path:** design email-body ingestion or another Substack-specific full-text path before exposing any Substack pasted-text upgrade UI.
4. **Dev-only audit cleanup:** full local audit still reports dev-only moderate advisories. Not production-shipped and not blocking, but schedule as dependency hygiene.
5. **Android phone smoke:** earlier physical APK smoke remains unresolved because no phone was connected. Re-run if Android confidence is required before mobile/offline work.

### Open questions / decisions needed

1. Should `codex/v0.8.1-next-security-patch` become the canonical base for new features, or should it be merged to `main` first?
2. Is the next feature Substack email-body ingestion, Android offline/library work, or another item from the capture-quality roadmap?
3. Should dev-only audit cleanup be handled before the next feature branch, or deferred?

### Session self-critique

- The first v0.8.1 backup command had a quoting bug and created an empty file. Integrity/schema check caught it immediately; the bad artifact was removed and a valid backup was taken before deploy.
- The first API smoke cleanup tried to touch `chunks_vec` without loading `sqlite-vec`, causing `no such module: vec0`. Cleanup was rerun safely through Node with `sqlite-vec`; reuse that pattern.
- I almost stopped at "high Next advisory fixed" after `next@16.2.9`, but production audit still had moderate PostCSS through Next. The explicit PostCSS override was needed to make production audit actually clean.
- Telegram smoke used owner-shaped webhook updates, not a manual phone message. It validates production webhook/code paths but is not identical to human Telegram UX.
- This entry is dense because it covers many rapid reports/plans/releases. The next agent should consolidate source-of-truth docs before adding more planning layers.

### Action items for the next agent

1. Inspect deployed source from `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades`; branch `codex/v0.8.1-next-security-patch`, HEAD `8d7faf669df2545481a80c68784e416169b2f5b7`.
2. Avoid using dirty `phase2` for new edits until its unrelated modified/untracked files are reviewed.
3. For any production-affecting change: take DB backup, run local gates, deploy with `BRAIN_DEPLOY_HEALTH_TOKEN_SOURCE=remote`, verify health/providers, run API + Telegram-shaped smoke, and clean smoke records.
4. For vector cleanup, use Node with `sqlite-vec.load(db)`.
5. Use `/Users/arun.prakash/.codex/skills/adversarial-review/` for future adversarial reviews.

### State snapshot

- **Current phase / version:** v0.8.1 production security patch deployed; v0.8 capture-quality release shipped and smoke green.
- **Active release branch:** `codex/v0.8.1-next-security-patch` in `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades`, HEAD `8d7faf669df2545481a80c68784e416169b2f5b7`.
- **Previous v0.8 branch:** `codex/v0.8.0-safe-capture-upgrades`, commit `8d689eb07d14afbf0da4718df15c1c14fb3e416e`.
- **Phase2 local branch:** `codex/v0.7.7-deployment-hygiene`, dirty with modified docs/APK config and untracked reports/plans/spike scripts.
- **Production runtime:** `brain.service` active; `https://brain.arunp.in` health OK with auth; Next `16.2.9`; production audit `0`; providers OK.
- **Production DB/artifacts after cleanup:** zero items, zero dummy-title items, zero artifacts, zero enrichment jobs, zero embedding jobs, zero chunks/chunks_vec, zero Telegram updates, zero metadata cache, zero artifact dirs.
- **Next milestone:** choose canonical branch/base, then begin new feature work from the deployed v0.8.1 state.

---

## 2026-06-11 08:24 IST - Entry #62 - Started v0.8.2 Capture Review Inbox from deployed v0.8.1 base

**Entry author:** AI agent (Codex) · **Triggered by:** user asked Codex to pick up from this running log, then said `execute` after the first local v0.8.2 Review slice was built and verified.

### Planned since last entry

Entry #61 left v0.8.1 deployed and smoke-green from the clean temp worktree `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades`, while warning that the `phase2` worktree was intentionally dirty and should not be reset. It listed the next milestone as choosing a canonical branch/base and beginning new feature work from the deployed v0.8.1 state.

During pickup, Codex found an additional later report in the dirty `phase2` worktree:

- `ReviewReport/AI_BRAIN_COMPETITOR_RESEARCH_FEATURE_RECOMMENDATIONS_2026-06-10_23-46-33_IST.md`

That report recommended the next release shape as `v0.8.2 Capture Review And Selected Text`, starting with a Capture Review / Needs Attention surface before selected-text capture.

### Done

- Confirmed local state matched the previous handoff:
  - `phase2` remains on `codex/v0.7.7-deployment-hygiene` with modified/untracked docs and should remain a preservation zone.
  - `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades` was clean on `codex/v0.8.1-next-security-patch`, HEAD `8d7faf669df2545481a80c68784e416169b2f5b7`.
  - `origin/main` is an ancestor of v0.8.1; v0.8.1 is 2 commits ahead of `origin/main`.
- Created branch `codex/v0.8.2-capture-review` in `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades` from deployed v0.8.1.
- Added testable Review attention logic:
  - `src/lib/review/attention.ts` classifies captures needing attention.
  - Reasons include pasted-text upgrade, Substack preview, generic metadata-only, failed capture, failed summary, failed semantic indexing, missing semantic chunks, and duplicate source URLs.
  - `listAttentionItems()` queries items with duplicate counts, chunk counts, and embedding state.
  - `summarizeAttentionItems()` produces counts for the Review summary row.
- Added focused tests in `src/lib/review/attention.test.ts` covering:
  - YouTube metadata-only -> Add text.
  - Substack `paywall_preview` -> preview review without pasted-text upgrade.
  - duplicate + processing/indexing failures.
  - full-text items marked done but missing chunks.
  - strong fully indexed captures not appearing in Review.
  - summary-count aggregation.
- Built the Review UI:
  - New route `src/app/review/page.tsx`.
  - Summary stats for Need text, Failures, Search gaps, and Duplicates.
  - Empty state when nothing needs attention.
  - Review rows showing attention labels, platform/quality/time/char metadata, plain-English reason text, primary action, Open item, Source, and Delete.
  - Primary `Add text` action links directly to the existing item upgrade form anchor.
- Added delete support for Review rows:
  - `src/app/review/actions.ts` validates `item_id`, deletes the item, and revalidates `/` and `/review`.
- Enabled Review navigation:
  - `src/components/sidebar.tsx` now enables `/review` in desktop and mobile nav.
  - `src/components/command-palette.tsx` adds `Go to Review`.
- Updated `src/app/items/[id]/upgrade-text-form.tsx`:
  - Form now has `id="upgrade-text"`.
  - Textarea id changed to `upgrade-text-input` to avoid duplicate IDs.

### Cross-lane notes

- All source edits were made only in `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades` on `codex/v0.8.2-capture-review`.
- The dirty `phase2` worktree was not reset, cleaned, merged, or used for source edits.
- No production deploy, DB migration, server change, or remote smoke was performed.
- Local dev server was started at `http://127.0.0.1:3010`; it was still running at handoff time. The server logs showed repeated local enrichment-provider backoff messages because the dev worker started, but no production state was touched.

### Learned

- The competitor-research report created after entry #61 is the clearest current product breadcrumb: build Capture Review first, then Browser Selected-Text Capture.
- The existing v0.8 upgrade policy already has the right distinction for Substack: `paywall_preview` belongs in Review, but does not expose pasted-text upgrade until a Substack email/body capture path exists.
- The in-app Browser control tool was not exposed in this Codex session even though the app browser was open. Runtime verification used authenticated local `curl` against `/review` instead.
- A plain `brain-session` cookie is enough to pass the local proxy presence check for server-render smoke, matching the existing proxy behavior.

### Deployed / Released

Nothing deployed, staged, committed, pushed, or released this session.

### Documents created or updated this period

**Created:**
- `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/src/lib/review/attention.ts` - Review attention classifier/query/summarizer.
- `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/src/lib/review/attention.test.ts` - focused review classifier tests.
- `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/src/app/review/page.tsx` - Review / Needs Attention page.
- `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/src/app/review/actions.ts` - Review row delete server action.

**Updated:**
- `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/src/components/sidebar.tsx` - enabled Review nav.
- `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/src/components/command-palette.tsx` - added Go to Review.
- `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/src/app/items/[id]/upgrade-text-form.tsx` - added stable `#upgrade-text` anchor.
- `Phase_2_RUNNING_LOG.md` - this entry.

### Verification

- `node --import tsx --test src/lib/review/attention.test.ts` passed: 6 tests.
- `npm run typecheck` passed.
- `npm run lint` passed with the same two known pre-existing warnings:
...
- `npm run test` passed: 459 tests, 0 failures.
- Runtime route checks:
  - Unauthenticated `GET /review` returned redirect as expected.
  - Authenticated local `GET /review` with a dummy `brain-session` cookie returned `200` and rendered the empty Review state.
  - A temporary local dev DB row titled `Codex Review Runtime Test` verified non-empty Review row rendering, `Add text` action to `/items/<id>#upgrade-text`, Source link, Open item, and Delete button.
  - Temporary runtime-test row was removed; follow-up count was `0`.

### Current remaining to-do

1. Review the v0.8.2 local diff in `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades`:
...
2. Decide whether to commit the Review slice as-is or add a client-side "mark good enough" action before commit.
3. If committing, use branch `codex/v0.8.2-capture-review`; suggested commit title: `feat(capture): add review inbox`.
4. After commit, decide whether to continue v0.8.2 with Browser Selected-Text Capture on the same branch or split it into a follow-up branch.
5. Before any production deploy, repeat the v0.8 release discipline: production DB backup, local gates, deploy with `BRAIN_DEPLOY_HEALTH_TOKEN_SOURCE=remote`, health/provider checks, API + Telegram-shaped smoke, and cleanup verification.

### Open questions / decisions needed

1. Should Review include a persistent "mark good enough" state in v0.8.2, or is opening/upgrading/deleting enough for the first slice?
2. Should duplicate review rows only flag exact `source_url` matches, or should canonical URL normalization be added before this ships?
3. Should Browser Selected-Text Capture be part of the same v0.8.2 release branch or a separate v0.8.3 slice?

### Session self-critique

- I did not get a visual screenshot because the Browser control tools were not exposed in this session. I verified by local authenticated HTML/runtime checks instead; a future agent should still visually inspect `/review` in the in-app browser before shipping.
- The Review delete button is immediate and uses the existing permanent delete behavior. That matches current app patterns, but a future recycle-bin/undo feature would improve safety.
- The temporary dev DB fixture was inserted with direct SQLite for runtime verification because TS eval imports did not work cleanly. It was removed and verified, but use a proper smoke helper if this pattern repeats.
- The Review page uses exact `source_url` for duplicate detection. This is intentionally narrow for the first slice and may miss normalized duplicates.

### Action items for the next agent

1. Open `http://127.0.0.1:3010/review` in the app browser and visually inspect desktop/mobile layout if Browser controls are available.
2. Run `git diff --stat` and review the seven changed/new source files in `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades`.
3. If no issues, commit on `codex/v0.8.2-capture-review`.
4. If continuing feature work before commit, add either "mark good enough" or Browser Selected-Text Capture, but avoid mixing both without an explicit plan.
5. Keep `phase2` dirty state intact; do not reset it to make the log look cleaner.

### State snapshot

- **Current phase / version:** v0.8.2 Capture Review started locally; v0.8.1 remains deployed in production.
- **Active worktree:** `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades`
- **Active branch:** `codex/v0.8.2-capture-review`
- **Branch base:** `8d7faf669df2545481a80c68784e416169b2f5b7` (`fix(deps): patch next security advisory`)
- **Working tree:** dirty with uncommitted v0.8.2 Review source edits; no staging performed.
- **Local runtime:** Next dev server running at `http://127.0.0.1:3010`; `/review` rendered with authenticated local check.
- **Production runtime:** unchanged from entry #61; v0.8.1 deployed, production not touched this session.
- **Next milestone:** review/commit Capture Review Inbox, then choose whether selected-text capture joins v0.8.2 or becomes the next slice.

---

## 2026-06-11 08:37 IST - Entry #63 - v0.8.2 Review Inbox committed; Browser Selected-Text Capture implemented and committed

**Entry author:** AI agent (Codex) · **Triggered by:** User asked Codex to "execute" and then "proceed" from Entry #62.

### Planned since last entry

Entry #62 left v0.8.2 Capture Review Inbox implemented but uncommitted, with a decision still open on whether Browser Selected-Text Capture should join the same branch. The user then approved continuing execution.

### Done

- Committed the Capture Review Inbox slice on `codex/v0.8.2-capture-review`:
  - `59111b1 feat(capture): add review inbox`
  - Includes the `/review` route, attention classifier/tests, Review delete action, sidebar/command-palette navigation, and stable `#upgrade-text` item anchor.
- Implemented Browser Selected-Text Capture on the same branch:
  - `extension/src/background.ts` adds a selection-only context menu action: `Save selected text to Brain`.
  - `extension/src/capture.ts` sends `selected_text` and `x-brain-capture-source: extension`.
  - `src/app/api/capture/url/route.ts` accepts selected text, validates passage length before item creation, and treats selected text as incoming `client_dom` quality.
  - `src/lib/capture/selected-text.ts` creates `browser_selected_text` captures with `Provided by: browser selection` provenance.
  - `src/lib/capture/capture-url.ts` routes selected text before remote extraction.
  - `src/lib/capture/user-provided.ts` now supports configurable provenance/body headings while keeping pasted-text defaults.
  - `src/lib/capture/upgrade-policy.ts` allows selected browser text to upgrade weak `metadata_only` or `paywall_preview` captures for YouTube, YouTube Shorts, LinkedIn, Substack, and generic articles without overwriting stronger full-text captures.
  - `extension/README.md` documents right-click page capture and selected-text capture.
- Added focused tests for selected text:
  - new extension-source selected text item creation without page fetch.
  - selected text upgrading an existing Substack preview.
  - too-short selected text rejected without creating an item.
  - upgrade policy accepts selected text for weak captures and rejects overwriting strong full text.
- Fixed extension dependency audit during validation:
  - `extension/package-lock.json` now resolves `@crxjs/vite-plugin` to `2.5.0` and `rollup` to `2.80.0`.
  - `npm audit --json` in `extension/` returned zero vulnerabilities after the lockfile update.
  - Root lockfile version in `extension/package-lock.json` now matches `extension/package.json` at `0.6.2`.
- Added `extension/dist/**` to `eslint.config.mjs` global ignores because the extension production build created generated files that root lint was scanning.
- Committed Browser Selected-Text Capture:
  - `80bb253 feat(capture): add browser selected text capture`

### Cross-lane notes

- All source edits and commits were made only in `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades` on branch `codex/v0.8.2-capture-review`.
- The dirty `phase2` worktree was not reset, cleaned, merged, or used for source edits.
- Production was not touched.
- Local dev server was still running at `http://127.0.0.1:3010/review` from Entry #62.

### Learned

- Chrome extension context menus support a `selection` context and expose selected text through `OnClickData.selectionText`, which fits the desired right-click selected-passage capture flow.
- The extension audit failure came from `@crxjs/vite-plugin@2.4.0` pulling vulnerable `rollup@2.79.2`; `npm audit fix` moved the lockfile to `@crxjs/vite-plugin@2.5.0` and patched `rollup@2.80.0`.
- Selected browser text should be treated as `client_dom`, not pasted full text, so it can upgrade weak browser/server captures while preserving the provenance distinction.

### Deployed / Released

Nothing deployed, pushed, tagged, or released this session. Local git commits only:
...
**Created:**
- `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/src/lib/capture/selected-text.ts` - selected browser-text capture builder.

**Updated:**
- `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/src/app/api/capture/url/route.ts` - selected-text API handling and validation.
- `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/src/app/api/capture/url/route.test.ts` - selected-text route coverage.
- `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/src/lib/capture/capture-url.ts` - selected-text extraction routing.
- `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/src/lib/capture/upgrade-policy.ts` - selected-text weak-capture upgrade rules.
- `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/src/lib/capture/upgrade-policy.test.ts` - selected-text upgrade policy tests.
- `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/src/lib/capture/user-provided.ts` - configurable pasted/selected text provenance headings.
- `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/extension/src/background.ts` - selected-text context menu.
- `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/extension/src/capture.ts` - selected-text payload support.
- `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/extension/README.md` - extension usage docs.
- `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/extension/package-lock.json` - extension audit fix.
- `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades/eslint.config.mjs` - ignore generated extension build output.
- `Phase_2_RUNNING_LOG.md` - this follow-up entry.

### Verification

- `node --import tsx --test src/app/api/capture/url/route.test.ts src/lib/capture/upgrade-policy.test.ts src/lib/capture/user-provided.test.ts` passed: 21 tests.
- `npm run build` in `extension/` passed.
- `npm audit --json` in `extension/` passed with 0 vulnerabilities.
- `npm run typecheck` passed.
- `npm run lint` passed with the same two known pre-existing warnings:
  - `src/lib/client/register-sw.ts`
  - `src/lib/queue/enrichment-batch-cron.ts`
- `npm run build` passed with the existing `unpdf` warning.
- `npm run check:build-artifacts` passed: no `.next/standalone/data` directory.
- `npm run test` passed: 463 tests, 0 failures.
- `git diff --check` passed.
- Final source worktree check after committing was clean on `codex/v0.8.2-capture-review`.

### Current remaining to-do

1. Visually inspect `http://127.0.0.1:3010/review` in the in-app browser before shipping if Browser controls are available.
2. Manually load/reload the unpacked extension from `extension/dist` and smoke right-click selected-text capture into the local app.
3. Decide whether v0.8.2 should ship with both commits as-is or add "mark good enough" before release.
4. If shipping v0.8.2, run production release discipline from Entry #61: production DB backup, local gates, deploy with `BRAIN_DEPLOY_HEALTH_TOKEN_SOURCE=remote`, health/provider checks, API + Telegram-shaped smoke, and cleanup verification.
5. Push/open PR only after the user confirms remote publication.

### Open questions / decisions needed

1. Should Review include a persistent "mark good enough" state before v0.8.2 ships, or are open/upgrade/delete enough for this release?
2. Should duplicate review detection stay exact-URL-only for v0.8.2, or should canonical URL normalization be added now?
3. Should the selected-text extension build be manually tested in Chrome before release, or is API/unit/build coverage enough for this local branch checkpoint?

### Session self-critique

- Selected-text capture has strong API and policy coverage, but it still needs a real browser-extension smoke with the unpacked `extension/dist` build.
- The in-app Browser control tool still was not available in this Codex session; `/review` should get one visual pass before production release.
- `extension/package-lock.json` changed substantially because `npm audit fix` updated a transitive plugin path; the change is justified by the Rollup advisory but should still be reviewed before release.

### Action items for the next agent

1. Start from `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades` on branch `codex/v0.8.2-capture-review`.
2. Confirm `git status --short --branch` is clean and latest commits are `80bb253` then `59111b1`.
3. Open `/review` visually and smoke selected-text capture through the extension if possible.
4. Decide whether to add "mark good enough"; if not, prepare v0.8.2 release notes around Review Inbox plus Browser Selected-Text Capture.
5. Do not reset the dirty `phase2` worktree; it remains intentionally untouched.

### State snapshot

- **Current phase / version:** v0.8.2 local implementation checkpoint complete; v0.8.1 remains deployed in production.
- **Active worktree:** `/private/tmp/ai-brain-v0.8.0-safe-capture-upgrades`
- **Active branch:** `codex/v0.8.2-capture-review`
- **Latest commits:** `80bb253 feat(capture): add browser selected text capture`; `59111b1 feat(capture): add review inbox`
- **Working tree:** clean after commits.
- **Local runtime:** Next dev server still running at `http://127.0.0.1:3010/review`.
- **Production runtime:** unchanged from Entry #61; production not touched this session.
- **Next milestone:** visual/extension smoke, then decide whether to ship v0.8.2 or add "mark good enough" first.
